...
using namespace std;
/**
 * pass by reference 从参数文件路径中获得各种文件名称的vector
 */
...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "Usage: ./vdo_slam path_to_settings path_to_sequence" << endl;
        return 1;
    }
    ...
    {
        cerr << endl
             << "Different number of images for depth map." << endl;
        return 1;
    }
    else if (vstrFilenamesSEM.size() != vstrFilenamesRGB.size())
    {
        cerr << endl
             << "Different number of images for segmentation." << endl;
        return 1;
    }
    else if (vstrFilenamesFLO.size() != vstrFilenamesRGB.size())
    {
        cerr << endl
             << "Different number of images for flow map." << endl;
        return 1;
    }
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        {
            cerr << endl
                 << "Failed to load image at: " << vstrFilenamesRGB[ni] << endl;
            return 1;
        }
              ...
              vector<double> &vTimestamps, vector<cv::Mat> &vPoseGT, vector<vector<float>> &vObjPoseGT)
{
    ...
    while (!fPose.eof())
    {
        ...
        if (!s.empty())
        {
            ...
            ss >> Pose_tmp.at<float>(0, 0) >> Pose_tmp.at<float>(0, 1) >> Pose_tmp.at<float>(0, 2) >> Pose_tmp.at<float>(0, 3) >> Pose_tmp.at<float>(1, 0) >> Pose_tmp.at<float>(1, 1) >> Pose_tmp.at<float>(1, 2) >> Pose_tmp.at<float>(1, 3) >> Pose_tmp.at<float>(2, 0) >> Pose_tmp.at<float>(2, 1) >> Pose_tmp.at<float>(2, 2) >> Pose_tmp.at<float>(2, 3) >> Pose_tmp.at<float>(3, 0) >> Pose_tmp.at<float>(3, 1) >> Pose_tmp.at<float>(3, 2) >> Pose_tmp.at<float>(3, 3);
    ...
    while (!fObjPose.eof())
    {
        ...
        if (!s.empty())
        {
            ...
            ss >> ObjPose_tmp[0] >> ObjPose_tmp[1] >> ObjPose_tmp[2] >> ObjPose_tmp[3] >> ObjPose_tmp[4] >> ObjPose_tmp[5] >> ObjPose_tmp[6] >> ObjPose_tmp[7] >> ObjPose_tmp[8] >> ObjPose_tmp[9];
