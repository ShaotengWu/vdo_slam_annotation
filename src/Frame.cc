     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mThDepthObj(thDepthObj)
{
    ...
    if(mvKeys.empty())
        return;
    // # Use sampled feature or detected feature for background (1: sampled, 0: detected) sampled是随机采集的特征点
    // demo中是 0 
    if (UseSampleFea==0)
    {
        // // // Option I: ~~~~~~~ use detected features ~~~~~~~~~~ // // //
        // 这里遍历的是特征点，后面物体上遍历的是像素 ？这些是用来和下一帧做匹配的，当前帧和上一帧对应的点在上一帧里存着？
        ...
        for (int i = 0; i < mvKeys.size(); ++i)
        {
            ...
            if (imDepth.at<float>(y,x)>mThDepth || imDepth.at<float>(y,x)<=0)  // new added in Aug 21 2019
                continue;
            // 是x y各自方向上的差，光流的存储形式是每一个位置存了Vec2f来表示deltaX deltaY
            // 只用了ORB的特征点，并没有用到descriptor 但是整幅图像的光流全被提取了 
...
void Frame::UndistortKeyPoints()
{
    ...
    // Undistort points 这里reshape是什么意思？？？？
    mat=mat.reshape(2);
...
cv::Mat Frame::UnprojectStereoStat(const int &i, const bool &addnoise)
{
    ...
    if(z>0)
    {
        ...
        //// using ground truth
        // 对于第二帧，也就是初始化后面的第一帧，调用这个函数的时候是lastFrame，也就是调用了初始化那一帧的这个函数，位姿是单位的
