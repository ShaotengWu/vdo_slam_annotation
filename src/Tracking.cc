                                ...
                                const double &timestamp, cv::Mat &imTraj, const int &nImage)
{
    ...
    if(mState!=NO_IMAGES_YET)
    {
        ...
        */
        ...
        // 构造函数中存储的值存在 mvStatKeysTmp里面，存储的是下一帧中可以被光流跟踪到的点在当前帧的像素位置，已经被赋予了当前帧的深度，同样也在tmp里面
        // 这里要做的是从上一帧中拿出来当前帧从上一帧光流跟踪到的点
        ...
        // 这里为什么舍去了
        // mvTmp保存的是当前帧通过Frame构造函数提取的一些信息
    ...
    mCurrentFrame.nSemPosi_gt.resize(vObjPose_gt.size()); // vector<int>
    mCurrentFrame.vObjPose_gt.resize(vObjPose_gt.size()); // vector<cv::Mat>
...
void Tracking::Initialization()
{
    ...
    // initialize the 3d points
    {
        ...
        // mvStatKeysTmp 是当前帧在Frame构造函数中提取的ORB静态特征点
    ...
    // Lastframe里存储的是LastFrame与它的下一帧光流跟踪的点
    mLastFrame.mvStatKeys = mCurrentFrame.mvStatKeysTmp; // new added Jul 30 2019
    mLastFrame.mvStatDepth = mCurrentFrame.mvStatDepthTmp;  // new added Jul 30 2019
...
cv::Mat Tracking::GetInitModelCam(const std::vector<int> &MatchId, std::vector<int> &MatchId_sub)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        // mvStatKeys里存储的是这一帧从上一帧光流跟踪过来的点 =Last.Corres
        cv::Point2f tmp_2d;
    ...
    // solve
    int iter_num = 500;
    // 重投影误差小于0.4时认为时inliers
    double reprojectionError = 0.4, confidence = 0.98; // 0.5 0.3
    // 这里可以返回inliners 和后面的motionmodel作对比
